...
const ComparisionAnalysis = () => {
  ...
  if (status === 'loading') {
   return <div class="Talentloading"><img  src={loading} alt="Loading"/></div>;
  }
 
  if (status === 'failed') {
    return <p class="Talentloading">{error}</p>;
  }
  ...
  {sortedUsers.map((user, index) => (
    // Iterate over each skill of the current user
    Object.entries(user.skills[0] || {}).map(([skill, level], skillIndex) => (
      <tr key={`${index}-${skillIndex}`}>
        {/* Render the name only on the first row for each user */}
        {/* {skillIndex === 0 && (
          <td rowSpan={Object.keys(user.skills[0] || {}).length}>{user.name}</td>
        )} */}
        <td>{user.name}</td>
        <td>{user.lead}</td>
        <td>{user.account}</td>
